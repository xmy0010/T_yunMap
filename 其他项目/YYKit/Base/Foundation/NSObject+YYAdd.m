...
+ (BOOL)swizzleInstanceMethod:(SEL)originalSel with:(SEL)newSel {
    ...
    method_exchangeImplementations(class_getInstanceMethod(self, originalSel), // exchangeInstanceMethodImplementations
...
+ (BOOL)swizzleClassMethod:(SEL)originalSel with:(SEL)newSel {
    ...
    if (!originalMethod || !newMethod) return NO;
    method_exchangeImplementations(originalMethod, newMethod); // exchangeClassMethodImplemntations
    return YES;
}
...
- (id)deepCopyWithArchiver:(Class)archiver unarchiver:(Class)unarchiver { // deepCopy
    id obj = nil;
    @try {
