...
{
    return [_db executeUpdate:@"delete from TbMovie where identifier=?", movie.identifier];
}

- (NSArray *)getAllMovies
{
    NSMutableArray *empsArray = [NSMutableArray array];
    FMResultSet *rs = [_db executeQuery:@"select * from TbMovie"];
    while ([rs next]) {
